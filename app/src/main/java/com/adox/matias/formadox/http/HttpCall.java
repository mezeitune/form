...
public class HttpCall extends Thread {// a esta clase , la extiendo de thread
	...
	public enum CallType {POST, GET, JSON};
	...
	public void runGet() {
		...
		try {
			...
			this.response = EntityUtils.toString(ent, HTTP.UTF_8);
	...
	public void runPostJSON() {
		...
		try {
            ...
            // Creo objeto JSON
            jsonResponse = new JSONObject(response); //Creamos un objeto JSON a partir de la cadena


            if (this.callbackHandler != null)
				this.callbackHandler.onResponse(this.accion, jsonResponse,
						this.paramsMap);
	 ...
	 * 
	 * Este mtodo est mal implementado, ya que es bloqueante y no pasivo por
	 ...
	 * dicho handler como parmetro en la inicializacin de la llamada.
	 * */
